...
elif test $board = ls1012ardb -o $board = ls1046ardb -o $board = ls1088ardb; then
...
if test $board = ls1012ardb -o $board = ls1012afrwy -o $board = ls1046ardb -o $board = ls1088ardb; then
    # probe QSPI flash
    sf probe
fi
...
# DPAA2 MC firmware
if test $board = ls1088ardb -o $board = ls2088ardb; then
    if load $bd_type $bd_part $img_load_addr $mcfw_img && $pt_off_cmd $addr_mcfw $efs && $erase_cmd $addr_mcfw +$filesize && $write_cmd $img_load_addr $addr_mcfw $filesize; then
	echo Success: flashed $mcfw_img to $addr_mcfw
    else
	echo Failed to flash $mcfw_img
...
elif test $board = ls1088ardb -o $board = ls2088ardb; then
